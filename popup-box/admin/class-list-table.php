...
class Wow_List_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$title         = ! empty( $item['title'] ) ? $item['title'] : '<em>' . esc_attr__( 'Untitle', 'popup-box' ) . '</em>';
		...
		$actions       = array(
			'edit'      => '<a href="' . esc_url( $edit_url ) . '">' . esc_attr__( 'Edit', 'popup-box' ) . '</a>',
			'duplicate' => '<a href="' . esc_url( $duplicate_url ) . '" style="color:green;">' . esc_attr__( 'Duplicate', 'popup-box' )
			               . '</a>',
			'delete'    => '<a href="' . esc_url( wp_nonce_url($delete_url, $slug . '_nonce' ) ) . '" style="color:red;">' . esc_attr__( 'Delete', 'popup-box' ) . '</a>',
		);

		return '<a href="' . esc_url( $edit_url ) . '">' . sanitize_text_field( $title ) . '</a>' . $this->row_actions( $actions );
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'title'  => 'Title',
			'test'   => esc_attr__( 'Test Mode', 'popup-box' ),
			'status' => esc_attr__( 'Status', 'popup-box' ),
		);
	...
	private function table_data() {
		...
		if ( $result ) {
			foreach ( $result as $key => $value ) {
				...
				$test_on        = '<span class="has-text-danger has-text-weight-semibold">' . esc_attr__( 'On', 'popup-box' ) . '</span>';
				$test_off       = esc_attr__( 'Off', 'popup-box' );
				$test           = ! empty( $param['test'] ) ? $test_on : $test_off;
				$tooltip_off    = esc_attr__( 'Click for Deactivate the popup.', 'popup-box' );
				$tooltip_on     = esc_attr__( 'Click for Activate the popup.', 'popup-box' );
				...
				$status_on      = '<a href="' . esc_url( $deactivate_url ) . '" class="button is-info is-small has-tooltip-bottom" data-tooltip="' . esc_attr( $tooltip_off ) . '">' . esc_attr__( 'Activated', 'popup-box' ) . '</a>';
				$status_off     = '<a href="' . esc_url( $activate_url ) . '" class="button is-danger is-small has-tooltip-bottom" data-tooltip="' . esc_attr( $tooltip_on ) . '">' . esc_attr__( 'Deactivated', 'popup-box' ) . '</a>';
	...
	public function get_search() {
		...
		$search = sanitize_text_field( urldecode( trim( $_POST['s'] ) ) );
	...
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="%1$s[]" value="%2$s" />', 'ID', absint( $item['ID'] ) );
	}
	...
	public function get_bulk_actions() {
		$actions = array(
			'delete-items' => esc_attr__( 'Delate', 'popup-box' ),
		);
